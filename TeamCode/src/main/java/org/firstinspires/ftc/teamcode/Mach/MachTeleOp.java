...
public class MachTeleOp extends MachHardware {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            motorPower = normalizeArray(motorPower,temp,weight, suggestedRange,15);
            setPower();
        }
    }
	/* For debugging
    ...
    private void drivePOV(double forward, double right, double rotation, double range) {
        ...
        motorPower[0] = Range.clip(forward + right - rotation, -range, range);
        motorPower[1] = Range.clip(forward - right + rotation, -range, range);
        motorPower[2] = Range.clip(forward - right - rotation, -range, range);
        motorPower[3] = Range.clip(forward + right + rotation, -range, range);
    }
